...
struct PrintOpConversion
  ...
  splitBlock(Operation *op, Value cnd, Location loc,
             ConversionPatternRewriter &rewriter) {
    ...
    if (cnd) {
      ...
      ifBlock = rewriter.splitBlock(prevBlock, op->getIterator());
  ...
  static void llPrintf(Operation *op, Value msg, ValueRange args, Value cnd,
    ...
    auto [ifBlock, thenBlock] = splitBlock(op, cnd, loc, rewriter);
    if (ifBlock) {
